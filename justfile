...
install-deps target=detected_target:
  ...
  if ! just check-deps; then
    {{ if target == "x86_64-unknown-linux-gnu" { "just install-deps-linux-x86_64" } else if target == "aarch64-unknown-linux-gnu" { "just install-deps-linux-aarch64" } else if (target == "x86_64-apple-darwin" || target == "aarch64-apple-darwin") { "just install-deps-macos" } else { error("unsupported arch") } }}
